...
<?php
    ...
    $result = mysqli_query($con,"select count(publisher_id) from publisher inner join admin on admin.admin_id = publisher.admin_id");
...
<?php
    $row_list = array();
    $result = mysqli_query($con,"select*from publisher where admin_id = $admin_id limit $start, $end");
...
if(isset($_POST["form-add-publisher"])){

    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $email = $_POST['email'];

    $query = "insert into publisher (fname,lname,email,admin_id) values ('$fname','$lname','$email',$admin_id)";

    mysqli_query($con,$query);

    header('Location:publisher.php?page='.$page);

}else if(isset($_POST["form-update-publisher"])){
    ...
    $query = "update publisher set fname='$fname', lname='$lname', email='$email' where publisher_id=$publisher_id";

    mysqli_query($con,$query);

    header('Location:publisher.php?page='.$page);

}else if(isset($_POST["form-delete-publisher"])){
...
<body background="resources/img/download.jpeg" style="background-size: 100%;">
                        ...
                        <?php 
                            ...
                            foreach($row_list as $row){
                                echo '<tr>';
                                    ...
                                    require "components/modals/publisher/publisher-delete-modal.php";
                    ...
                    <ul class="pagination"> 
                        <?php
                            for($i=1;$i<=$required_pages;$i++){
                                echo '<li class="page-item"><a class="page-link" href="publisher.php?page='.$i.'">'.$i.'</a></li>';
                            }
                        ?>
                    </ul>
                </nav>
    ...
    <?php
        require "components/modals/publisher/publisher-add-modal.php";
    ?>
</body>
</html>